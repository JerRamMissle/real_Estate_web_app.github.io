...
const PropertyDetails = ({ propertyDetails }) => {
  // Check if `propertyDetails` is available; if not, show a fallback message.
  if (!propertyDetails) {
...
export async function getStaticProps({ params: { id } }) {
  ...
  return {
    props: {
      propertyDetails: data || null, // Return null if data is unavailable
    },
  };
}
...
export async function getStaticPaths() {
  ...
  return {
    paths: paths || [],
    fallback: false, // Only pre-rendered pages are generated at build time
  };
}
